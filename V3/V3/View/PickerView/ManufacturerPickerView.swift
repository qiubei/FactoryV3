...
class ManufacturerPickerView: PickerView, UIPickerViewDataSource, UIPickerViewDelegate {

    override init(frame: CGRect) {
        super.init(frame: frame)
        _titleLabel.text = "生产商"
        _pickerView.dataSource = self
        _pickerView.delegate = self

        _saveButton.bk_addEventHandler({ [weak self] _ in
            guard let `self` = self else { return }
            var info = DeviceInfo.shared.production
            info = self._data[self._pickerView.selectedRow(inComponent: 0)]
            DeviceInfo.shared.production = info
            self.dismiss()
        }, for: .touchUpInside)
    }
